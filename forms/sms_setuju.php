...
if($hasil){
            ...
            while($data= mysqli_fetch_array($hasil)){
                ...
                //$get_nopengirim = str_replace("+62", "0", $noPengirim);
                ...
                if(strlen($jam_booking)=="4" and strlen($tgl_booking)=="10"){

                    if($pecah[0]=="Booking" || $pecah[0]=="booking" || $pecah[0]=="BOOKING"){

                        if($pecah[1] !="" and $pecah[2] !="" and $pecah[3] !="" and $pecah[4] !="")

                        {


                                $hari_get = date('D',strtotime($pecah[2]));
                                $dayList = array(
                                            'Sun' => 'Minggu',
                                            'Mon' => 'Senin',
                                            'Tue' => 'Selasa',
                                ...
                                $pesan_error="format sms : Booking#nama_kelas#tgl_booking#jam, contoh: Booking#kode_ruangan#2016-03-12#kode_jam";
                                $query=mysqli_query($conn,"INSERT INTO outbox (DestinationNumber,

                                TextDecoded) VALUES ('".$noPengirim."', '$pesan_error')");
                       ...
                       $query3 = mysqli_query($conn,"UPDATE inbox SET Processed = 'true' WHERE ID = '$id'");

                        //echo $jam_booking;

                }else{
                    $pesan_error="Silahkan periksa format sms,\nformat sms : Booking#nama_kelas#tgl_booking#jam, contoh: Booking#kode_ruangan#2016-03-12#kode_jam";
                    $query=mysqli_query($conn,"INSERT INTO outbox (DestinationNumber,

                    TextDecoded) VALUES ('".$noPengirim."', '$pesan_error')");
                    echo "error panjang format";
                    //echo strlen($jam_booking);
